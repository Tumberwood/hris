										...
										<table id="tblhpyemtd_karyawan" class="table table-striped table-bordered table-hover nowrap" width="100%">
											...
											<tfoot>
												<tr>
													...
													<th id="karyawan_120"></th>
													<th id="karyawan_121"></th>
													<th id="karyawan_122"></th>
													<th id="karyawan_123"></th>
												</tr>
											</tfoot>
										</table>
									</div> <!-- end of table -->
										...
										<table id="tblhpyemtd_kontrak" class="table table-striped table-bordered table-hover nowrap" width="100%">
											...
											<tfoot>
												<tr>
													...
													<th id="kontrak_120"></th>
													<th id="kontrak_121"></th>
													<th id="kontrak_122"></th>
													<th id="kontrak_123"></th>
												</tr>
											</tfoot>
										</table>
									</div> <!-- end of table -->
										...
										<table id="tblhpyemtd_kbm_reg" class="table table-striped table-bordered table-hover nowrap" width="100%">
											...
											<tfoot>
												<tr>
													...
													<th id="kbm_reg_120"></th>
													<th id="kbm_reg_121"></th>
													<th id="kbm_reg_122"></th>
													<th id="kbm_reg_123"></th>
												</tr>
											</tfoot>
										</table>
									</div> <!-- end of table -->
...
// --------- start _detail --------------- //
			...
			tblhpyemtd_karyawan = $('#tblhpyemtd_karyawan').DataTable( {
				...
				columns: [
					...
					// 		return $.fn.dataTable.render.number(',', '.', 0).display(terimaLainBaru);
					// 	}
					// },
...

{
    ...
    className: 'btn btn-success',
    name: 'btnExcelBerwarna',
    action: function () { // Tidak perlu async di sini
        let timerInterval;
        ...
        Swal.fire({
            ...
            didOpen: () => {
                ...
                // DELAY PROSES BERAT AGAR UI & STOPWATCH BISA DILENDER DULU
                setTimeout(async () => {
                    try {
                        const workbook = new ExcelJS.Workbook();
                        const worksheet = workbook.addWorksheet('Payroll');
                        ...
                        // =========================
                        const compareMap = {
                            18: 'gp', 20: 't_jab', 22: 'terima_lain', 24: 'var_cost',
                            26: 'tj_khusus', 28: 'fix_cost', 30: 'premi_abs', 32: 'lembur15',
                            34: 'rp_lembur15', 36: 'lembur2', 38: 'rp_lembur2', 40: 'lembur3',
                            42: 'rp_lembur3', 44: 'total_lembur_jam_final', 46: 'total_rp_lembur',
                            48: 'komp_rekontrak', 50: 'cuti_tahunan', 52: 'cuti_bersama',
                            54: 'sisa_cuti_hari', 56: 'komp_sisa_cuti', 58: 'thr', 60: 'pot_makan',
                            62: 'c_pot_upah', 64: 'pot_upah', 66: 'c_pot_resign', 68: 'pot_resign',
                            70: 'c_pot_jam', 72: 'pot_jam', 74: 'pendapatan_lain_before_pph',
                            76: 'pot_lain_before_pph', 78: 'bpjs_kes_perusahaan', 80: 'jkk',
                            82: 'jkm', 84: 'bruto', 86: 'persen_ter', 88: 'pot_pph21',
                            90: 'after_pph21', 92: 'jht_perusahaan', 94: 'jp_perusahaan',
                            96: 'pot_jht_karyawan', 98: 'pot_jp_karyawan', 100: 'bpjs_kes_karyawan',
                            102: 'bpjs_kes_perusahaan', 104: 'jkk', 106: 'jkm', 108: 'pot_piutang',
                            110: 'denda_apd', 112: 'iuran_spsi', 114: 'pendapatan_lain_after_pph',
                            116: 'pot_lain_after_pph', 118: 'gaji_bersih', 120: 'bulat', 122: 'gaji_terima'
                        };
                        ...
                        $('#tblhpyemtd_karyawan thead th').each(function (idx) {
                            ...
                            if ($(this).hasClass('lama')) {
                                cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFEBAA' } };
                            } else if ($(this).hasClass('baru')) {
                                cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFC3E6CB' } };
                            } else {
                        ...
                        allRows.forEach(function (data) {
                            ...
                            // compareField() -> bg-danger
                            Object.entries(compareMap).forEach(([colIndex, field]) => {
                                const nilaiCocokan = Number(rowObj?.hpyemtd_cocokan?.[field] ?? 0);
                                const nilaiAsli = Number(rowObj?.hpyemtd?.[field] ?? 0);

                                if (nilaiCocokan !== nilaiAsli) {
                                    const cell = excelRow.getCell(Number(colIndex) + 1);
                                    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFDC3545' } };
                                    cell.font = { color: { argb: 'FFFFFFFF' }, bold: true };
                                    const selisih = nilaiCocokan - nilaiAsli;
                                    cell.note = `Selisih : ${formatNumber(selisih)}`;
                                }
                            });
                        });
                        ...
                        $('#tblhpyemtd_karyawan thead th').each(function (idx) {
                            const th = $(this);
                            const col = worksheet.getColumn(idx + 1);
                            col.width = Math.max(15, Math.min(40, th.text().length + 5));

                            if (th.hasClass('text-right')) col.alignment = { horizontal: 'right' };
                            if (th.hasClass('text-center')) col.alignment = { horizontal: 'center' };

                            if (th.hasClass('text-danger')) {
                                col.eachCell(function (cell, rowNumber) {
                                    if (
                                        rowNumber > 1 &&
                                        !(cell.fill && cell.fill.fgColor && cell.fill.fgColor.argb === 'FFDC3545')
                                    ) {
                ...
                }, 200); // Berikan jeda 200ms agar browser sempat merender UI modal
            },
            willClose: () => {

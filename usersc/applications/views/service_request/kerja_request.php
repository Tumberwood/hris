...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			edtservice_request = new $.fn.dataTable.Editor( {
				ajax: {
					url: "../../models/service_request/kerja_request.php",
					type: 'POST',
					data: function (d){
				...
				fields: [ 
					...
					{
						label: "Rencana Pengerjaan <sup class='text-danger'>*<sup>",  
						name: "service_request.rencanapengerjaan",
						type: "datetime",
						def: function () { 
							return new Date(); 
						},
					...
					{
						label: "Estimasi Waktu <sup class='text-danger'>*<sup>",
						name: "service_request.estimasiwaktu",
						type: "datetime",
						def: function () { 
							return new Date(); 
						},
					...
					{
						label: "Waktu Selesai",
						name: "service_request.waktuselesai",
						type: "datetime",
						def: function () { 
							return new Date(); 
						},
            ...
            edtservice_request.on( 'preSubmit', function (e, data, action) {
				if(action != 'remove'){
					rencanapengerjaan = edtservice_request.field('service_request.rencanapengerjaan').val();
					if(!rencanapengerjaan || rencanapengerjaan == ''){
						edtservice_request.field('service_request.rencanapengerjaan').error( 'Wajib diisi!' );
					}
					estimasiwaktu = edtservice_request.field('service_request.estimasiwaktu').val();
					if(!estimasiwaktu || estimasiwaktu == ''){
						edtservice_request.field('service_request.estimasiwaktu').error( 'Wajib diisi!' );
					}
				}

		...
		if ($sheetData[0][0] == 'Nama Level') {
			try{
				...
				for($i = 1; $i < count($sheetData); $i++){
					...
					// mapping komponen
					$komponen = [
						['id_hpcxxmh' => 32,   'nominal' => $sheetData[$i][1]],
						['id_hpcxxmh' => 33,   'nominal' => $sheetData[$i][2]],
					];

					// cari Level
					$qs = $db->query('select','hevxxmh')
					...
					foreach($komponen as $k){
						...
						$qs_check = $db->query('select','htpr_hevxxmh')
							->get(['id'])
							->where('id_hevxxmh',$id_hevxxmh)
			...
			$data = array(
				"message" => "Template Upload Komponen Level salah!",
				"type_message" => "danger"
			);
		}
	}else{

<?php
	...
	if(isset($_FILES['filename']['name']) && in_array($_FILES['filename']['type'], $file_mimes)) {
		...
		if ($sheetData[0][4] == 'Premi Absen') {
			try{
				...
				for($i = 1; $i < count($sheetData); $i++){
					...
					$skala_upah = $sheetData[$i][1];
					$sub_tipe = $sheetData[$i][2];
				...
				if (count($emptyPeg) >= 1) {
					...
					$data = array(
						"message" => "Upload Komponen Premi Absen Berhasil.</br>" .$dataupload. " data berhasil di import.</br>" . $datakembar. " data kembar TIDAK di import.",
			...
			$db->commit();
				...
				$db->rollback();
				$data = array(
					"message" => "Upload Komponen Premi Absen gagal," . $e,
					"type_message" => "danger"
				);
			}
		} else {
		...
		$data = array(
			"message" => "Upload Komponen Premi Absen gagal, format file salah!",
			"type_message" => "danger"
		);
	}

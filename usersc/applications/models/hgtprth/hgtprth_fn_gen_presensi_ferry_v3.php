    ...
    try{
            ...
            $qs_htsprrd_new = $db
                ...
                ->exec(' WITH presensi AS (
                            ...
                            LEFT JOIN (
                                -- overtime
                                SELECT
                                    MAX(tanggaljam_awal_toleransi_lembur) AS tanggaljam_awal_toleransi_lembur,
                                    MAX(tanggaljam_awal_toleransi_lembur_premi) AS tanggaljam_awal_toleransi_lembur_premi,
                                    jam_awal_lembur,
                                    ot.id_hemxxmh,
                                    is_pot_ti,
                                ...
                                FROM(
                                        SELECT
                                            ...
                                            DATE_SUB(CONCAT(hto.tanggal, " ", hto.jam_awal), INTERVAL 5 MINUTE) tanggaljam_awal_toleransi_lembur_premi,
                        ...
                        perhitungan AS (
                            ...
                            END AS pot_jam_makan_manual,
                                ...
                                IF(id_htsxxmh = 1, 0,
                                    IF( (tanggal_jam_izin_awal_in IS NOT NULL AND is_potong_gaji_in = 0) OR (kode_absen IS NOT NULL), 0, -- KALAU ADA IZIN TIDAK POT GAJI, MAKA 0
                                        IF(IFNULL(tanggaljam_awal_toleransi_lembur, "") = "" OR st_clock_in = "LATE",
                                            IF(id_htsxxmh IN (5, 12, 71, 77) AND is_sabtu = 1,
                                ...
                                -- hitung pot_early
                                IF( (tanggal_jam_izin_akhir_out IS NOT NULL AND is_potong_gaji_out = 0) OR (kode_absen IS NOT NULL) OR id_htsxxmh = 1, 0, -- KALAU ADA IZIN TIDAK POT GAJI, MAKA 0
                                    IF(id_htsxxmh IN (5, 12, 71, 77) AND is_sabtu = 1,
        ...
        $qu_pengecualian = $db
            ...
            ->exec(' UPDATE htsprrd AS a
                    ...
                    WHERE a.tanggal = :tanggal AND DAYOFWEEK(:tanggal) = 7 AND id_hemxxmh IN (130, 208) AND a.status_presensi_in = "AL"
                    '
        );

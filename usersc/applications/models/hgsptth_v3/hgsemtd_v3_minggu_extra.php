			...
			$qi_schedule = $editor->db()
				...
				->exec(' INSERT INTO htssctd
						...
						WITH RECURSIVE date_range AS (
							SELECT DATE(:tanggal_awal) AS minggu
							UNION ALL
							SELECT DATE_ADD(minggu, INTERVAL 1 DAY)
							FROM date_range
							WHERE minggu < :tanggal_akhir
						)
						SELECT
							c.minggu,
							a.id_hemxxmh,
							...
							concat(c.minggu, " ", TIME(DATE_SUB(d.jam_awal, INTERVAL d.menit_toleransi_awal_in MINUTE))) AS tanggaljam_awal_t1,
							concat(c.minggu, " ", d.jam_awal) AS tanggaljam_awal,
							CONCAT(
								CASE
									WHEN DATE_ADD(d.jam_awal, INTERVAL d.menit_toleransi_akhir_in MINUTE) >= "24:00:00" THEN DATE_ADD(c.minggu, INTERVAL 1 DAY)
									ELSE c.minggu
								END,
								" ",
								TIME(
							...
							CASE
								WHEN d.kode like "malam%" AND d.jam_akhir <= "12:00:00"
								THEN CONCAT(DATE_ADD(c.minggu, INTERVAL 1 DAY), " ", d.jam_akhir)
								ELSE CONCAT(c.minggu, " ", d.jam_akhir)
							END AS tanggaljam_akhir,
							CONCAT(
								CASE
									WHEN DATE_ADD(d.jam_akhir, INTERVAL d.menit_toleransi_akhir_out MINUTE) >= "24:00:00" 
										OR d.kode like "malam%" AND d.jam_akhir <= "12:00:00" THEN DATE_ADD(c.minggu, INTERVAL 1 DAY)
									ELSE c.minggu
								END,
								" ",
								TIME(
							...
							CASE
								...
								THEN CONCAT(DATE_ADD(c.minggu, INTERVAL 1 DAY), " ", d.jam_awal_istirahat)
								ELSE CONCAT(c.minggu, " ", d.jam_awal_istirahat)
							END AS tanggaljam_awal_istirahat,
							CASE
								WHEN d.kode like "malam%" AND d.jam_akhir_istirahat <= "12:00:00"
								THEN CONCAT(DATE_ADD(c.minggu, INTERVAL 1 DAY), " ", d.jam_akhir_istirahat)
								ELSE CONCAT(c.minggu, " ", d.jam_akhir_istirahat)
							END AS tanggaljam_akhir_istirahat
						FROM hgsemtd_v3 AS a
						LEFT JOIN hgsptth_v3 AS b ON b.id = a.id_hgsptth_v3
						LEFT JOIN date_range AS c ON c.minggu BETWEEN b.tanggal_awal AND b.tanggal_akhir
						LEFT JOIN htsxxmh AS d ON d.id = a.id_htsxxmh
						WHERE a.nama = "minggu" AND DAYOFWEEK(c.minggu) = 1 AND a.id_hemxxmh = :id_hemxxmh;
						'
					);
		})
				...
				$qi_schedule = $editor->db()
					...
					->exec(' INSERT INTO htssctd
							...
							SELECT
								...
								END AS tanggaljam_akhir_istirahat
							FROM htsxxmh AS d 
							LEFT JOIN date_range AS c ON c.minggu BETWEEN :tanggal_awal AND :tanggal_akhir
							WHERE DAYOFWEEK(c.minggu) = 1 and d.id = :id_htsxxmh_new;
							'
					);
			}
		})

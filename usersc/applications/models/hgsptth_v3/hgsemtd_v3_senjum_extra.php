			...
			$qi_schedule = $editor->db()
				...
				->exec(' INSERT INTO htssctd
						...
						WITH RECURSIVE date_range AS (
							SELECT DATE(:tanggal_awal) AS senjum
							UNION ALL
							SELECT DATE_ADD(senjum, INTERVAL 1 DAY)
							FROM date_range
							WHERE senjum < :tanggal_akhir
						)
						SELECT
							c.senjum,
							a.id_hemxxmh,
							...
							concat(c.senjum, " ", TIME(DATE_SUB(d.jam_awal, INTERVAL d.menit_toleransi_awal_in MINUTE))) AS tanggaljam_awal_t1,
							concat(c.senjum, " ", d.jam_awal) AS tanggaljam_awal,
							CONCAT(
								CASE
									WHEN DATE_ADD(d.jam_awal, INTERVAL d.menit_toleransi_akhir_in MINUTE) >= "24:00:00" THEN DATE_ADD(c.senjum, INTERVAL 1 DAY)
									ELSE c.senjum
								END,
								" ",
								TIME(
							...
							CASE
								...
								THEN CONCAT(DATE_ADD(c.senjum, INTERVAL 1 DAY), " ", TIME(DATE_SUB(d.jam_akhir, INTERVAL d.menit_toleransi_akhir_out MINUTE)))
								ELSE CONCAT(c.senjum, " ", TIME(DATE_SUB(d.jam_akhir, INTERVAL d.menit_toleransi_akhir_out MINUTE)))
							END AS tanggaljam_akhir_t1,
							CASE
								WHEN d.kode like "malam%" AND d.jam_akhir <= "12:00:00"
								THEN CONCAT(DATE_ADD(c.senjum, INTERVAL 1 DAY), " ", d.jam_akhir)
								ELSE CONCAT(c.senjum, " ", d.jam_akhir)
							END AS tanggaljam_akhir,
							CONCAT(
								CASE
									WHEN DATE_ADD(d.jam_akhir, INTERVAL d.menit_toleransi_akhir_out MINUTE) >= "24:00:00" 
										OR d.kode like "malam%" AND d.jam_akhir <= "12:00:00" THEN DATE_ADD(c.senjum, INTERVAL 1 DAY)
									ELSE c.senjum
								END,
								" ",
								TIME(
							...
							CASE
								...
								THEN CONCAT(DATE_ADD(c.senjum, INTERVAL 1 DAY), " ", d.jam_awal_istirahat)
								ELSE CONCAT(c.senjum, " ", d.jam_awal_istirahat)
							END AS tanggaljam_awal_istirahat,
							CASE
								WHEN d.kode like "malam%" AND d.jam_akhir_istirahat <= "12:00:00"
								THEN CONCAT(DATE_ADD(c.senjum, INTERVAL 1 DAY), " ", d.jam_akhir_istirahat)
								ELSE CONCAT(c.senjum, " ", d.jam_akhir_istirahat)
							END AS tanggaljam_akhir_istirahat
						FROM hgsemtd_v3 AS a
						LEFT JOIN hgsptth_v3 AS b ON b.id = a.id_hgsptth_v3
						LEFT JOIN date_range AS c ON c.senjum BETWEEN b.tanggal_awal AND b.tanggal_akhir
						LEFT JOIN htsxxmh AS d ON d.id = a.id_htsxxmh
						WHERE a.nama = "senjum" AND DAYOFWEEK(c.senjum) BETWEEN 2 AND 6 AND a.id_hemxxmh = :id_hemxxmh;
						'
					);
		})
				...
				$qi_schedule = $editor->db()
					...
					->exec(' INSERT INTO htssctd
							...
							SELECT
								...
								END AS tanggaljam_akhir_istirahat
							FROM htsxxmh AS d 
							LEFT JOIN date_range AS c ON c.senjum BETWEEN :tanggal_awal AND :tanggal_akhir
							WHERE DAYOFWEEK(c.senjum) BETWEEN 2 AND 6 and d.id = :id_htsxxmh_new;
							'
					);
			}
		})

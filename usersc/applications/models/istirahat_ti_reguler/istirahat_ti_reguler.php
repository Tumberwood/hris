	...
	->exec('
		WITH history AS (
			...
			FROM (
				...
				FROM hemjbrd
				LEFT JOIN hemxxmh
					ON hemxxmh.id = hemjbrd.id_hemxxmh
				WHERE
		...
		hasil_union AS (
			...
			SELECT
				...
				CASE
					...
					WHEN pot_jam_istirahat > 0
					THEN "durasi kerja < 7/8 jam"
			...
			WHERE
				...
				-- yang potongan jam karena early, late dsb
				-- ini agar tidak masuk
				AND (
					a.is_pot_premi <> 1
			...
			/* =========================================================
			   QUERY 2
			   PRIORITAS KEDUA
			   QUERY DENGAN CASE WHEN LENGKAP
			   ========================================================= */
			...
			SELECT
				...
				CASE
					...
					WHEN DAYNAME(a.tanggal) = "Friday"
						AND a.st_jadwal LIKE "%PAGI%"
					THEN "AMAN"

					WHEN c.jumlah_grup = 2
						AND TIMESTAMPDIFF(
							MINUTE,
							a.break_in,
							a.break_out
						...
						AND a.pot_jam > 0
					THEN "4 Grup > 30 Menit"

					WHEN a.pot_jam_istirahat > 0
						AND TIMESTAMPDIFF(
							MINUTE,
							a.break_in,
							a.break_out
					...
					WHEN id_hodxxmh = 9
						AND st_jadwal LIKE "%06:00-%"
						AND (
							a.break_in BETWEEN
								jad.tanggaljam_awal_istirahat
								AND DATE_SUB(
									jad.tanggaljam_akhir_istirahat,
									INTERVAL 1 HOUR
								)
							OR
							a.break_out BETWEEN
					...
					WHEN ot.is_istirahat = 2
						...
						AND (
							TIME(a.break_in)
								NOT BETWEEN "11:00:00" AND "12:00:00"
							OR
							TIME(a.break_out)
								NOT BETWEEN "11:00:00" AND "12:00:00"
						)
					...
					WHEN ot.id IS NULL
						...
						AND (
							TIME(a.break_in)
								NOT BETWEEN "12:00:00" AND "13:00:00"
							OR
							TIME(a.break_out)
								NOT BETWEEN "12:00:00" AND "13:00:00"
						)
					...
					WHEN ot.is_istirahat = 2
						...
						AND (
							...
							(
								...
								AND (
									TIME(a.break_in)
										NOT BETWEEN "17:45:00" AND "19:00:00"
									OR
									TIME(a.break_out)
										NOT BETWEEN "17:45:00" AND "19:00:00"
								)
							...
							(
								...
								AND (
									TIME(a.break_in)
										NOT BETWEEN "18:00:00" AND "19:00:00"
									OR
									TIME(a.break_out)
										NOT BETWEEN "18:00:00" AND "19:00:00"
								)
					...
					WHEN ot.id IS NULL
						...
						AND (
							...
							(
								...
								AND (
									TIME(a.break_in)
										NOT BETWEEN "19:00:00" AND "20:00:00"
									OR
									TIME(a.break_out)
										NOT BETWEEN "19:00:00" AND "20:00:00"
								)
					...
					WHEN ot.is_istirahat = 2
						...
						AND (
							...
							/* =================
							   2. NORMAL
							   18 Aug 2025 s/d sebelum Ramadan
							   ================= */
							...
							(
								...
								AND (
									TIME(a.break_in)
										NOT BETWEEN "02:00:00" AND "03:00:00"
									OR
									TIME(a.break_out)
										NOT BETWEEN "02:00:00" AND "03:00:00"
								)
							...
							(
								...
								AND (
									TIME(a.break_in)
										NOT BETWEEN "03:00:00" AND "04:00:00"
									OR
									TIME(a.break_out)
										NOT BETWEEN "03:00:00" AND "04:00:00"
								)
							...
							(
								...
								AND (
									TIME(a.break_in)
										NOT BETWEEN "01:00:00" AND "02:00:00"
									OR
									TIME(a.break_out)
										NOT BETWEEN "01:00:00" AND "02:00:00"
								)
					...
					/* =================================================
					   YANG BREAK_IN / BREAK_OUT
					   DI LUAR RENTANG ISTIRAHAT
					   ================================================= */

					WHEN g.jam_awal_istirahat <> "00:00:00"
						AND jad.keterangan NOT LIKE "%TJ%"
						AND (
							a.break_in NOT BETWEEN
								jad.tanggaljam_awal_istirahat
				...
				/*
				 * QUERY 2 TIDAK MEMPUNYAI BAGIAN
				 * JADI TETAP NULL
				 */
				NULL AS bagian,
...
$results['data']['htsprrd'] = !empty($rs_htsprrd)
	? $rs_htsprrd
	: [];

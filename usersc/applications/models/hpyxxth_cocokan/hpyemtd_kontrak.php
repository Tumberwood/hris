	...
	if ( ! isset($_POST['id_hpyxxth']) || ! is_numeric($_POST['id_hpyxxth']) ) {
		...
		$editor = Editor::inst( $db, 'hpyemtd_cocokan' )
			...
			->fields(
				...
				Field::inst( 'IF(IFNULL(hpyemtd.premi_abs, 0) - IFNULL(hpyemtd_cocokan.premi_abs, 0) = 0, 0, IFNULL(hpyemtd.premi_abs, 0)) - IFNULL(hpyemtd_cocokan.total_rp_lembur, 0) + IFNULL(hpyemtd.total_rp_lembur, 0) + IF(IFNULL(hpyemtd.pot_upah, 0) - IFNULL(hpyemtd_cocokan.pot_upah, 0) = 0, 0, IFNULL(hpyemtd_cocokan.pot_upah, 0)) - IFNULL(hpyemtd.pot_jam, 0) - IFNULL(hpyemtd.pot_lain_before_pph, 0) AS hpyemtd.terima_lain' ),

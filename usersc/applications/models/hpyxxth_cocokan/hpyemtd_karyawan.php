	...
	if ( ! isset($_POST['id_hpyxxth']) || ! is_numeric($_POST['id_hpyxxth']) ) {
		...
		$editor = Editor::inst( $db, 'hpyemtd_cocokan' )
			...
			->fields(
				...
				Field::inst( 'IF( IFNULL(hpyemtd.premi_abs, 0) - IFNULL(hpyemtd_cocokan.premi_abs, 0) = 0, 0, IFNULL(hpyemtd.premi_abs, 0) ) - hpyemtd_cocokan.total_rp_lembur + hpyemtd.total_rp_lembur + (CASE WHEN ABS(IFNULL(hpyemtd_cocokan.pot_upah, 0) - IFNULL(hpyemtd.pot_upah, 0)) > 2 THEN hpyemtd_cocokan.pot_upah ELSE 0 END) - hpyemtd.pot_jam - hpyemtd.pot_lain_before_pph AS hpyemtd.terima_lain' ),

<?php 
     ...
     * Table terkait    : acara_m
     * Parameter        : 
     *  - id_acara_m_old       : data existing (untuk keperluan edit), dipakai di query self
     */
    ...
    if($_GET['id_acara_m_old'] > 0){
        $id_acara_m_old = $_GET['id_acara_m_old'];
    }else{
        $id_acara_m_old = 0;
    }
    ...
    if($id_acara_m_old > 0){
        $qs_acara_m_self = $db
            ->query('select', 'acara_m')
            ->get([
                'id as id',
                'nama as text'
            ])
            ->where('id', $id_acara_m_old )
            ->exec();
        $rs_acara_m_self = $qs_acara_m_self->fetchAll();
    }else{
        $rs_acara_m_self = [];
    }
    // END query self
    ...
    $qs_acara_m_all = $db
        ...
        ->offset($offset)
        ->exec();
    $rs_acara_m_all = $qs_acara_m_all->fetchAll();
    // END query options all except self

    // BEGIN menggabungkan options
    if(count($rs_acara_m_self) > 0){
        $rs_opt = array_merge($rs_acara_m_self, $rs_acara_m_all);
    }else{
        $rs_opt = $rs_acara_m_all;
    }

<?php 
    ...
    if($_GET['id_hpcatmh_old'] > 0){
        $id_hpcatmh_old = $_GET['id_hpcatmh_old'];
    }else{
        $id_hpcatmh_old = 0;
    }
    ...
    if($id_hpcatmh_old > 0){
        $qs_hpcatmh_self = $db
            ->query('select', 'hpcatmh')
            ->get([
                'id as id',
                'kode as text'
            ])
            ->where('id', $id_hpcatmh_old )
            ->exec();
        $rs_hpcatmh_self = $qs_hpcatmh_self->fetchAll();
    }else{
        $rs_hpcatmh_self = [];
    }
    ...
    $qs_hpcatmh_all = $db
        ...
        ->offset($offset)
        ->exec();
    $rs_hpcatmh_all = $qs_hpcatmh_all->fetchAll();
    // END query options all except self
    
    // BEGIN menggabungkan options
    if(count($rs_hpcatmh_self) > 0){
        $rs_opt = array_merge($rs_hpcatmh_self, $rs_hpcatmh_all);
    }else{
        $rs_opt = $rs_hpcatmh_all;
    }

<?php 
     ...
     * Table terkait    : holxxmd
     * Parameter        : 
     *  - id_holxxmd_old       : data existing (untuk keperluan edit), dipakai di query self
     */
    ...
    if($_GET['id_holxxmd_old'] > 0){
        $id_holxxmd_old = $_GET['id_holxxmd_old'];
    }else{
        $id_holxxmd_old = 0;
    }
    ...
    if($id_holxxmd_old > 0){
        $qs_holxxmd_self = $db
            ->query('select', 'holxxmd')
            ->get([
                'id as id',
                'nama as text'
            ])
            ->where('id', $id_holxxmd_old )
            ->exec();
        $rs_holxxmd_self = $qs_holxxmd_self->fetchAll();
    }else{
        $rs_holxxmd_self = [];
    }
    // END query self
    ...
    $qs_holxxmd_all = $db
        ...
        ->offset($offset)
        ->exec();
    $rs_holxxmd_all = $qs_holxxmd_all->fetchAll();
    // END query options all except self

    // BEGIN menggabungkan options
    if(count($rs_holxxmd_self) > 0){
        $rs_opt = array_merge($rs_holxxmd_self, $rs_holxxmd_all);
    }else{
        $rs_opt = $rs_holxxmd_all;
    }

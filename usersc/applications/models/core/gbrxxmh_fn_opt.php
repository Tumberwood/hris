<?php 
    /**
     * Digunakan untuk populate options data branch / cabang
     * Table terkait    : gbrxxmh
     * Parameter        : 
     *  - id_gbrxxmh_old       : data existing (untuk keperluan edit), dipakai di query self
     */
    require_once( "../../../../users/init.php" );
	require_once( "../../../../usersc/lib/DataTables.php" );
	require_once( "../../../../usersc/helpers/datatables_fn_debug.php" );

    // BEGIN definisi variable untuk fn_ajax_results.php
    $data      = array();
    $rs_opt    = array();
    $c_rs_opt  = 0;
    $morePages = 0;
    // END definisi variable untuk fn_ajax_results.php

    // BEGIN select2 pagination preparation
    $page        = $_GET['page'];
    $resultCount = 10;
    $offset      = ($page - 1) * $resultCount;
    // END select2 pagination preparation

    if($_GET['id_gbrxxmh_old'] > 0){
        $id_gbrxxmh_old = $_GET['id_gbrxxmh_old'];
    }else{
        $id_gbrxxmh_old = 0;
    }

    // BEGIN query options self.
    // Hanya dipanggil jika field ada nilai id nya
    if($id_gbrxxmh_old > 0){
        $qs_gbrxxmh_self = $db
            ->query('select', 'gbrxxmh')
            ->get([
                'gbrxxmh.id as id',
                'IF(
                    gbrxxmh.kode IS NULL, 
                    gbrxxmh.nama,
                    IF(
                        gbrxxmh.nama IS NULL, 
                        gbrxxmh.kode,
                        CONCAT(gbrxxmh.kode," - ",gbrxxmh.nama)
                    )
                ) as text'
            ])
            ->where('gbrxxmh..id', $id_gbrxxmh_old )
            ->exec();
        $rs_gbrxxmh_self = $qs_gbrxxmh_self->fetchAll();
    }else{
        $rs_gbrxxmh_self = [];
    }
    // END query options self

    // BEGIN query options all except self
    $qs_gbrxxmh_all = $db
        ->query('select', 'gbrxxmh')
        ->get([
            'gbrxxmh.id as id',
            'IF(
                gbrxxmh.kode IS NULL, 
                gbrxxmh.nama,
                IF(
                    gbrxxmh.nama IS NULL, 
                    gbrxxmh.kode,
                    CONCAT(gbrxxmh.kode," - ",gbrxxmh.nama)
                )
            ) as text'
        ])
        ->where('gbrxxmh.is_active',1)
        ->where('gbrxxmh.id', $id_gbrxxmh_old, '<>' )
        ->where( function ( $r ) {
            $q = $_GET['search'];
            $r
                ->where('gbrxxmh.kode', '%' . $q . '%', 'LIKE' )
                ->or_where('gbrxxmh.nama', '%' . $q . '%', 'LIKE' );
        } )
        ->limit($resultCount)
        ->offset($offset)
        ->exec();
    $rs_gbrxxmh_all = $qs_gbrxxmh_all->fetchAll();
    // END query options all except self
    
    // BEGIN menggabungkan options
    if(count($rs_gbrxxmh_self) > 0){
        $rs_opt = array_merge($rs_gbrxxmh_self, $rs_gbrxxmh_all);
    }else{
        $rs_opt = $rs_gbrxxmh_all;
    }
    $c_rs_opt = count($rs_opt);    
    // END menggabungkan options

    // BEGIN finalisasi paginasi select2
    $endCount  = $offset + $resultCount;
    $morePages = $endCount > $c_rs_opt;
    // END finalisasi paginasi select2
    
    // tampilkan results
    require_once( "../../../../usersc/helpers/fn_ajax_results.php" );

?>
<?php
	...
	if(isset($_FILES['filename']['name']) && in_array($_FILES['filename']['type'], $file_mimes)) {
		...
		try{
			...
			$db->commit();
			$data = array(
				"message" => "Upload Makan Berhasil.</br>" .$dataupload. " data berhasil di import.</br>" . $datakembar. " data kembar TIDAK di import.",
			...
			$db->rollback();
			$data = array(
				"message" => "Upload Makan gagal," . $e,
				"type_message" => "danger"
			);
		}
	}else{
		$data = array(
			"message" => "Upload Makan gagal, format file salah!",
			"type_message" => "danger"
		);
	}

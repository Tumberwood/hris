<?php
	...
	if(isset($_FILES['filename']['name']) && in_array($_FILES['filename']['type'], $file_mimes)) {
		...
		try{
			...
			$datakembar = 0;
			$dataupload = 0;
			processSheetData(1, $sheetData, $db, $dataupload, $datakembar, "STAFF");
						
			$db->commit();
			$data = array(
				"message" => "Upload Staff Berhasil.</br>" .$dataupload. " data berhasil di import.</br>" . $datakembar. " data kembar TIDAK di import.",
			...
			$db->rollback();
			$data = array(
				"message" => "Upload Staff gagal," . $e,
				"type_message" => "danger"
			);
		}
	}else{
		$data = array(
			"message" => "Upload Staff gagal, format file salah!",
			"type_message" => "danger"
		);
	}

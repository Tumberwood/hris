<?php 
	...
	$qs_table_lb = $db
		...
		->exec('SELECT
				...
				AND status_presensi_in = "lb"
				ORDER BY a.id_hemxxmh, a.tanggal;
	
	');
	$rs_table_lb = $qs_table_lb->fetchAll();
	...
	$qs_absen_lb = $db
		...
		->exec('SELECT
				...
				WHERE a.tanggal BETWEEN :start_date AND :end_date
				AND a.is_active = 1 AND a.id_hemxxmh = :id_hemxxmh
				AND (htlxxmh_kode = "lb" OR a.keterangan LIKE "lb%")
				ORDER BY a.id_hemxxmh, a.tanggal;
	
	');
	$rs_absen_lb = $qs_absen_lb->fetchAll();

	$data = [
		"message" => "Upload berhasil!",
		"type_message" => "success",
		"rs_table_lb" => $rs_table_lb,
		"rs_absen_lb" => $rs_absen_lb,
	];

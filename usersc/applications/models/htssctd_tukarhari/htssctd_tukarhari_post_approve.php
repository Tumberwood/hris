    ...
    try{
        ...
        if($state == 1) {
            ...
            if(count($idPegawai) > 0) {
                ...
                // TANGGAL LIBUR DIKEMBALIKAN KE JADWAL ASLI SEBELUM MENJADI OFF / LIBUR
                $db->raw()->exec("
                    UPDATE htssctd
                    SET 
                        is_active = 1
                    ...
                    AND is_active = 0
                    AND (
                        keterangan LIKE '%Cuti Bersama%'
                        OR keterangan LIKE '%Public Holiday%'
                    )
                    AND id_htsxxmh <> 1
                    AND id_hemxxmh IN ($idPegawaiIn)
                ");
                ...
                // Begin INSERT TANGGAL PENGGANTI JADI OFF
                $qr_tanggal_pengganti = $db
                    ...
                    ->exec('INSERT INTO htssctd
                        ...
                        AND htssctd.id_htsxxmh = 1
                        AND htssctd.id_hemxxmh IN ('.$idPegawaiIn.')
                    ');
                // END INSERT TANGGAL PENGGANTI JADI OFF
            }
            ...
            if(count($idPegawai) > 0) {
                ...
                // nonaktifkan jadwal aktif tanggal terpilih
                $db->raw()->exec("
                    UPDATE htssctd
                    SET is_active = 0
                    WHERE is_active = 1

<?php 
    ...
    if (isset($_GET['id_htssctd_tukarhari'])) {
        $id_htssctd_tukarhari = $_GET['id_htssctd_tukarhari'];
        $qs_section = $db
            ->query('select', 'hosxxmh_htssctd_tukarhari')
            ->get(['id_hosxxmh'])
            ->where('id_htssctd_tukarhari', $id_htssctd_tukarhari)
            ->exec();
        $rs_section = $qs_section->fetchAll();
        foreach ($rs_section as $key => $sec) {
            if (!empty($id_hosxxmh)) {
                $id_hosxxmh .= ","; // Add a comma if $id_hosxxmh is not empty
            }
            $id_hosxxmh .= $sec['id_hosxxmh']; // Append the id_hosxxmh value
        }
    }
    
    if ($id_hosxxmh != '') {
        $w_id_hosxxmh = '(' . $id_hosxxmh . ')';
        $s_id_hosxxmh = 'IN';
    } else {
        $w_id_hosxxmh = '(-1)';
        $s_id_hosxxmh = 'NOT IN';
    ...
    // print_r($w_id_hosxxmh);
    // print_r($s_id_hosxxmh);

<?php 
     ...
     * Parameter        : 
     *  - id_htsptth_old       : data existing (untuk keperluan edit), dipakai di query self
     */
    ...
    if($_GET['id_htsptth_old'] > 0){
        $id_htsptth_old = $_GET['id_htsptth_old'];
    }else{
        $id_htsptth_old = 0;
    }
    ...
    if($id_htsptth_old > 0){
        $qs_htsptth_self = $db
            ->query('select', 'htsptth_new')
            ->get([
                'id as id',
                'nama as text'
            ])
            ->where('id', $id_htsptth_old )
            ->exec();

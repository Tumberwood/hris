<?php 
    ...
    // print_r($_GET['id_heyxxmd']);
    if (isset($_GET['id_heyxxmd'])) {
        $id_heyxxmd = $_GET['id_heyxxmd'];
        if ($id_heyxxmd > 0) {
            if (strpos($id_heyxxmd, ',') !== false) {
                // Jika terdapat tanda koma, maka pecah string menjadi array
                $id_values = explode(',', $id_heyxxmd);
                $w_id_heyxxmd = '(' . implode(',', $id_values) . ')';
                $s_id_heyxxmd = 'IN';
            } else {
                // Jika hanya satu nilai atau bukan array, gunakan default
                $w_id_heyxxmd = '(' . $id_heyxxmd . ')';
                $s_id_heyxxmd = 'IN';
            }
        } else {
            $w_id_heyxxmd = '(-1)';
        ...
        // Handle the case where id_heyxxmd is not defined
        $w_id_heyxxmd = '(-1)';
        $s_id_heyxxmd = 'NOT IN';
    }
    ...
    $qs_hemxxmh_all = $db
        ...
        ->where('hemjbmh.id_heyxxmd', $w_id_heyxxmd, $s_id_heyxxmd, false )

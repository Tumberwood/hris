	...
	if(isset($_FILES['filename']['name']) && in_array($_FILES['filename']['type'], $file_mimes)) {
		...
		try{
			...
			for($i = 1;$i < count($sheetData);$i++){
				...
				$qu_htpr_hemxxmh = $db
					->query('update', 'htpr_hemxxmh' )
					->set('is_active',0)
					->where('htpr_hemxxmh.id_hemxxmh', $id_hemxxmh )
					->where('htpr_hemxxmh.id_hpcxxmh', $id_hpcxxmh )
					->where('htpr_hemxxmh.tanggal_efektif', $tanggal_efektif )
					->where('htpr_hemxxmh.is_active', 1 )
					->exec();

				// Cek apakah ada data yang sama
				$qs_htpr_hemxxmh = $db
					->query('select', 'htpr_hemxxmh' )
					->get([
						'htpr_hemxxmh.id as id_htpr_hemxxmh'
					] )
					->where('htpr_hemxxmh.id_hemxxmh', $id_hemxxmh )
					->where('htpr_hemxxmh.id_hpcxxmh', $id_hpcxxmh )
					->where('htpr_hemxxmh.tanggal_efektif', $tanggal_efektif )
					->where('htpr_hemxxmh.is_active', 1 )
					->exec();
				$rs_htpr_hemxxmh = $qs_htpr_hemxxmh->fetchAll();
				
				$c_rs_htpr_hemxxmh = count($rs_htpr_hemxxmh);
				
				// Jika tidak ada data kembar maka di insert
				if($c_rs_htpr_hemxxmh == 0){
					$qi_htpr_hemxxmh = $db
						->query('insert', 'htpr_hemxxmh')
						->set('id_hemxxmh',$id_hemxxmh)
						->set('id_hpcxxmh',$id_hpcxxmh)
						->set('nominal',$nominal)
						->set('tanggal_efektif',$tanggal_efektif)
						->set('keterangan', 'Komponen per Karyawan (Gaji Pokok & Gaji BPJS)')
						->exec();
